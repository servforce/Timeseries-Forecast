...
## 7 新增功能（已完成）
- **model_id 复用**：Finetune 返回 `model_id`，可在后续请求中直接复用模型预测（跳过再次微调）。
- **模型清理策略**：  
  - Zeroshot 使用临时目录，不落盘保存。  
  - Finetune 模型默认保留 14 天，后台定时任务自动清理（可配置保留天数与清理间隔）。
- **协变量类型支持**：新增 `category_cov_name`，指定分类协变量；未列入者按数值型协变量处理。
- **指标逻辑增强**：IC/IR 采用历史切分验证区间计算；无法计算时返回 0.0 并附 warnings。
- **前端结果对比**：Zero-shot 与 Finetune 可视化分为两个区域，避免切换模式导致结果消失。
- **前端刷新能力**：提供“刷新”按钮，一键清空上传与结果，便于重新预测。
- **模型复用交互**：支持“用上次/取消复用”，并在结果区展示可复制的 `model_id`。

## 8 后续新增功能（规划）
1. 模型管理接口（列出/删除/查询 model_id 状态与剩余保留时间）
2. 结果对比增强（指标差异、曲线差异、模型对比）
3. 非负输出策略（可选后处理/可配置开关）
4. 长任务异步化（队列 + 进度提示）
5. 对象存储对接（S3/MinIO 模型缓存与下载）
